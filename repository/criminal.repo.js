...
class CriminalRepository {
  ...
  async updateCriminal(id, updateData) {
    try {
      return await criminalModel.findByIdAndUpdate(id, updateData, { new: true });
    } catch (error) {
  ...
  async deleteCriminal(id) {
    try {
      return await criminalModel.findByIdAndUpdate(id, { isDeleted: true }, { new: true });
    } catch (error) {
  ...
  async getCriminalsByLocation({ lat, lng, radius = 5000, page = 1, limit = 10 }) {
    try {
      ...
      console.error('Error fetching criminals by location:', error.message);
      throw error;
    }
  }
}
